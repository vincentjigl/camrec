...
struct JglRecorder : public MediaRecorderBase {
    JglRecorder(const String16 &opPackageName);
    virtual ~JglRecorder();
    ...
    virtual status_t start(sp<MediaCodecSource> *source);
...
private:
    ...
    status_t setParamVideoSvcLayer(int32_t svcLayer);
    status_t setParamVideoSliceHeight(int32_t sliceHeight);
    ...
    JglRecorder(const JglRecorder &);
    JglRecorder &operator=(const JglRecorder &);
};
